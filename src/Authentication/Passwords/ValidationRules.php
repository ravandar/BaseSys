...
namespace AliKhaleghi\BaseSys\Authentication\Passwords;
...
class ValidationRules
{
    ...
    public function strong_password(string $value, ?string &$error1 = null, array $data = [], ?string &$error2 = null)
    {
        ...
        if (function_exists('jwt_user') && jwt_user()) {
            $user = user();
        } else {
    ...
    protected function buildUserFromRequest()
    {
        ...
        if($recover = service('request')->getVar('recover')) {

            if(filter_var($recover, FILTER_VALIDATE_EMAIL)) {
                $data['email'] = $recover;
            }
            else {
                $data['phone'] = $recover;
                $data['email'] = model("UserModel")->select('email')->where('phone', $recover)->first()->email;
            }
        }
